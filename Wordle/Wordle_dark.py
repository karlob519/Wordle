...
lighter_grey = (225, 225, 225)
red = (200, 0, 10)
black = (0, 0, 0)
white = (255, 255, 255)

bg_colour = darker_grey
kb_colour = black
    ...
    def animate(self, colour):
        ...
        while rotate < 26:
            ...
            if rotate < 29:
                ...
                if rotate < 15:
                    for i in range(2):
                        pygame.draw.rect(screen, grey, (x-i,y-i, width+2, height+2), True)
                pygame.display.update()
            else:
                None
...
def kb():
    ...
    for i in range(10):
        ...
        y = start_y
        text = keyboard[i]
        box = Box(x, y, text, light_grey, white, kb_box_width, kb_box_height, grid = False)
        boxes.append(box)
        c += 1 
    ...
    for i in range(9):
        if i == 0:
            ...
            text = 'ENTER'
            colour = light_grey
            button = Button(x, y, text, (colour, white), 1.5, enter)
            ...
            text = 'BACK'
            colour = light_grey
            button = Button(x, y, text, (colour, white), 1.5, back)
            buttons.append(button)
...
def grid():
    ...
    for i in range(6):
        for j in range(5):
            ...
            y = start_y + i*dy
            text = ' '
            box = Box(x, y, text, bg_colour, white, grid_box_width, grid_box_height)
...
def invalid_guess():
    ...
    message_display(text, 40, light_grey, WIN_X, WIN_Y)
    sfx.invalid()
    return
...
# Main game loop
def game_loop():   
    global courser, attempt_num, count 
    run = True
    while run:
        # Indicator
        if courser < 30:
